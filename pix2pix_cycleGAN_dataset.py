...
class TwoImageData(Dataset): 
    def __init__(self, isTrain = False, gpu = 5):
        ...
        self.ImageLabeldataset6, self.person_id_set6 = PairedImages("normal", path6, k6, "name")
        ...
        print(len(self.person_id_set1),len(self.person_id_set2),
              len(self.person_id_set3),len(self.person_id_set4),
              len(self.person_id_set5),len(self.person_id_set6))
        ...
        # self.dict_of_list[id] = {'IMAGE1': [path1], 'IMAGE2': [path5]} # OCT, retina
        self.dict_of_list_full_permutation = {}
        cnt = 0
        for key,value in self.dict_of_list.items(): # key:person_id ; value:{...}
            for i in range(len(self.dict_of_list[key]['IMAGE1'])):
                for j in range(len(self.dict_of_list[key]['IMAGE2'])):
                    cnt = cnt + 1
        print("number of full permutation OCT+fundus images: ", cnt)

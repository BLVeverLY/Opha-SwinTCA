...
from RFMiD import SingleLabel, pix2pix, cyclegan, diffusion
...
def train(args, model, sampler, train_dataloader, val_dataloader):
    ...
    for epoch in range(TRAIN_EPOCH_START, EPOCH_TRAIN): # args.epochs
        ...
        for image1, image2, label in batch:
            if args.model == 'swin_one_RFMiD':
                images = image1.cuda(non_blocking=True) # 会加载到args.gpu对应的GPU上
        ...
        if((epoch+1)%20 == 0): # 每20个epoch输出/记录一次loss和accuracy
            model.eval()
            val_acc = []
            with torch.no_grad():
                for i, (val_image1, val_image2, val_label) in enumerate(val_dataloader):
                # for image1, images2, label in tqdm(val_dataloader):
                    if args.model == 'swin_one_RFMiD':
                        val_outputs = model(val_image1.cuda(non_blocking=True))
                    else:
                        val_outputs = model(val_image1.cuda(non_blocking=True), val_image2.cuda(non_blocking=True))
                    val_acc.append(accuracy_score(val_label.numpy(), torch.max(val_outputs, 1)[1].cpu().numpy()))
...
def eval(args, model, dataloader):
    ...
    if args.checkpoint_load and args.eval:
        model.load_state_dict(torch.load(args.checkpoint_load)) # torch.load(path, map_location='cuda:0')
    else:
    ...
    with torch.no_grad():
        ...
        for image1, image2, label in tqdm(dataloader):
            if args.model == 'swin_one_RFMiD':
                images = image1.cuda(non_blocking=True) # 会加载到args.gpu对应的GPU上
                labels = label.cuda(non_blocking=True) # torch.Size([batch_size])
                outputs = model(images) # torch.Size([batch_size, num_classes])
            else:
                images1 = image1.cuda(non_blocking=True) # 会加载到args.gpu对应的GPU上
                images2 = image2.cuda(non_blocking=True)
...
if __name__ == '__main__':
    ...
    if args.model == 'pix2pix':
        train_dataset = pix2pix(data_folder="/home/newljy/RFMiD_pix2pix/train/images", label_path="train.csv")
        test_dataset = pix2pix(data_folder="/home/newljy/RFMiD_pix2pix/test/images", label_path="test.csv")
        val_dataset = pix2pix(data_folder="/home/newljy/RFMiD_pix2pix/val/images", label_path="val.csv")
    elif args.model == 'cyclegan':
        train_dataset = cyclegan(data_folder="/home/newljy/RFMiD_cyclegan/train/images", label_path="train.csv")
        test_dataset = cyclegan(data_folder="/home/newljy/RFMiD_cyclegan/test/images", label_path="test.csv")
        val_dataset = cyclegan(data_folder="/home/newljy/RFMiD_cyclegan/val/images", label_path="val.csv")
    elif args.model == 'diffusion':
        train_dataset = diffusion(data_folder="/home/newljy/RFMiD_All_Classes_Dataset/1. Original Images/a. Training Set", diffusion_csv = "/home/newljy/SD_gen_data/OCT_train.csv", label_path="train.csv")
        test_dataset = diffusion(data_folder="/home/newljy/RFMiD_All_Classes_Dataset/1. Original Images/c. Testing Set", diffusion_csv = "/home/newljy/SD_gen_data/OCT_test.csv", label_path="test.csv")
        val_dataset = diffusion(data_folder="/home/newljy/RFMiD_All_Classes_Dataset/1. Original Images/b. Validation Set", diffusion_csv = "/home/newljy/SD_gen_data/OCT_test.csv", label_path="val.csv")
    elif args.model == 'swin_one_RFMiD':
        train_dataset = SingleLabel(data_folder="/home/newljy/RFMiD_All_Classes_Dataset/1. Original Images/a. Training Set", label_path="train.csv")
        test_dataset = SingleLabel(data_folder="/home/newljy/RFMiD_All_Classes_Dataset/1. Original Images/c. Testing Set", label_path="test.csv")
        val_dataset = SingleLabel(data_folder="/home/newljy/RFMiD_All_Classes_Dataset/1. Original Images/b. Validation Set", label_path="val.csv")
    ...
    if args.distributed:
        ...
        val_dataloader = torch.utils.data.DataLoader(dataset=val_dataset, batch_size=args.batch_size, sampler=val_sampler)
    else:
    ...
    if args.model == 'pix2pix' or args.model == 'cyclegan' or args.model == 'diffusion':
